package crc8

var CCITTTable = [256]byte{
	,
}

// CCITT calculates the CRC-8 of a byte slice.
//
// Note that it cannot be inlined due to the 'range' call: if you care
// about performance, paste the loop into your own code.
func CCITT(in []byte) (out uint8) {
	for _, b := range in {
		out = CCITTTable[out^b]
	}
	return out
}

func CCITTFirst8(in []byte) (out uint8) {
	_ = in[7]
	return CCITTTable[in[7]^CCITTTable[in[6]^CCITTTable[in[5]^CCITTTable[in[4]^CCITTTable[in[3]^CCITTTable[in[2]^CCITTTable[in[1]^CCITTTable[in[0]]]]]]]]]
}
